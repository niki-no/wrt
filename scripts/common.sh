...
[ -d "${applications}/luci-app-tcpdump" ] && sed -i 's/\bTcpdump 流量监控\b/流量截取/g' ${applications}/luci-app-tcpdump/po/zh_Hans/tcpdump.po
[ -d "${applications}/luci-app-argon-config" ] && sed -i 's/\bArgon 主题设置\b/"主题设置"/g' ${applications}/luci-app-argon-config/po/zh_Hans/argon-config.po
[ -d "${applications}/luci-app-vlmcsd" ] && sed -i 's/\bKMS 服务器\b/KMS 服务/g' ${applications}/luci-app-vlmcsd/po/zh_Hans/vlmcsd.po
...
# 带宽监控
[ -d "${applications}/luci-app-nlbwmon" ] && sed -i 's/\b带宽监控\b/监控/g' ${applications}/luci-app-nlbwmon/po/zh_Hans/nlbwmon.po
