...
return view.extend({
	...
	render: function(hosts_duids_pools) {
		...
		so = ss.option(form.Value, 'note', '中文备注');
		so.rmempty = true;
		so.optional = true;
		so.datatype = 'string';
						...
						leases.map(function(lease) {
							...
							if (lease.macaddr) {
								...
								for (var i = 0; i < hostSections.length; i++) {
									var hostMacs = L.toArray(hostSections[i].mac);
									if (hostMacs.includes(lease.macaddr) && hostSections[i].note) {
										note = hostSections[i].note;
										break;
									}
								}
							}
