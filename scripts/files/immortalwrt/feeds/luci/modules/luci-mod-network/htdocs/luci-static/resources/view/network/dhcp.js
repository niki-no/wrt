		...
		return m.render().then(function(mapEl) {
			poll.add(function() {
				return callDHCPLeases().then(function(leaseinfo) {
					...
					var macToNote = {};
					var hostSections = uci.sections('dhcp', 'host');
					for (var i = 0; i < hostSections.length; i++) {
						if (hostSections[i].mac && hostSections[i].note) {
							var macs = L.toArray(hostSections[i].mac);
							for (var j = 0; j < macs.length; j++) {
								var mac = macs[j].toUpperCase();
								macToNote[mac] = hostSections[i].note;
							}
						}
					}
					...
					cbi_update_table(mapEl.querySelector('#lease_status_table'),
						leases.map(function(lease) {
							...
							var note = '-';
							if (lease.macaddr) {
								var macUpper = lease.macaddr.toUpperCase();
								note = macToNote[macUpper] || '-';
							}
